...
cluster_data <- read_csv("data/EW_oa_to_segments_plus_geos_plus_voteshare.csv") |>
  transmute(oa21cd,
            low_diversity = -f1,
            security = f2)
...
#Write to file
d_cluster_segments |>
  select(oa21cd, low_diversity, security, segment, segment_name) |>
  write_csv("output/fixed/england_and_wales_oa_segmentation.csv")
